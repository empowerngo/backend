...
exports.createPurpose = async (parameter) => {
  ...
  const result = await sequelize.query(query, {
    replacements: [parameter.ngoID, parameter.projectID, parameter.purposeName, parameter.createdBy],
...
exports.updatePurpose = async (parameter) => {
  ...
  `;
  const result = await sequelize.query(query, {
    replacements: [parameter.purposeName, parameter.purposeID, parameter.ngoID, parameter.projectID],
    type: QueryTypes.UPDATE,
  });

  return result[1] > 0
    ? { status: true, statusCode: 200, message: "Purpose updated successfully." }
    : { status: false, statusCode: 404, message: "Purpose not found or no changes made." };
};

// Retrieve purposes for a project
exports.getPurposes = async (ngoID, projectID) => {
  const query = `
    SELECT PURPOSE_ID, PURPOSE_NAME, CREATED_DATE, UPDATED_DATE 
    FROM TB_NGO_PURPOSE WHERE NGO_ID = ? AND PROJECT_ID = ?;
  `;
  const result = await sequelize.query(query, {
    replacements: [ngoID, projectID],
