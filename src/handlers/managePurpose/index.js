  ...
  try {
    ...
    if (user.role !== 2) {
      return getResponseObject({
        ...
        message: "Only NGO Admin can manage purposes.",
      });
    }
    ...
    if (parameter.reqType === "s") {
      response = await service.createPurpose(parameter);
    } else if (parameter.reqType === "u") {
      response = await service.updatePurpose(parameter);
    } else if (parameter.reqType === "g") {
      response = await service.getPurposes(parameter.ngoID, parameter.projectID);
    } else {
      return getResponseObject({
        status: false,
        statusCode: HTTP_CODE.BAD_REQUEST,
    ...
    console.error("Error in managePurposes handler:", error);
    return getResponseObject({
      status: false,
