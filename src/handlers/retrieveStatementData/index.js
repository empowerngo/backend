  ...
  try {
    ...
    if (!authResponse || authResponse.role !== 2) {
      return getResponseObject({
        ...
        message: "Only admins can retrieve statement data.",
      });
    }
    ...
    let result = await service.fetcAllRecords(parameter.ngoID);
    ...
    console.error("Error in retrieve statement handler:", error);
    return getResponseObject({
      status: false,
